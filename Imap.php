...
class Imap {
     ...
     * @return array
     */
    public function getMessages($withbody = true, $sender_email) {
        $count = $this->countMessages();
        $emails = array();
        for ($i=$count;$i>=$count-100;$i--) {
            $emails[]= $this->formatMessage($i, $withbody,$sender_email);
        }
     ...
     * @param bool|true $withbody without body
     */
    public function getMessage($id, $withbody = true, $sender_email) {
        return $this->formatMessage($id, $withbody, $sender_email);
    }
     ...
     * @return array
     */
    protected function formatMessage($id, $withbody=true, $sender_email) {
        ...
        // Check if sender_email is provided and matches the sender's email address
        if ($sender_email !== ($header->from[0]->mailbox . '@' . $header->from[0]->host)) {
            return null;
        }
